...
const actions = [
  ...
  },
  {
    ...
    async run(p, ctx) {
      const st = await ctx.vrControl('settings.state');
      if (!st.connected) throw new Error(st.error || 'SteamVR is not running');
      await ctx.vrControl('bounds.force', { on: resolve(p.mode, st.boundsForced === true) });
    },
  },
  {
