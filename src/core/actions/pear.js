...
const { REPEAT_ORDER } = require('../pear');
...
const actions = [
  ...
  },
  {
    ...
    async run(p, ctx) {
      if (p.mode && p.mode !== 'cycle') await ctx.pearApi((c) => c.setRepeat(p.mode));
  ...
  },
  {
    ...
    async run(p, ctx) {
      ...
      await ctx.pearApi((c) => c.setVolume(p.mode === 'set' ? amount : c.sliderVolume() + (p.mode === 'down' ? -amount : amount)));
    },
  },
  {
    id: 'pear.seek',
    ...
    async run(p, ctx) {
      ...
      await ctx.pearApi((c) => c.seekBy(p.direction === 'back' ? -s : s));
    },
  },
];
