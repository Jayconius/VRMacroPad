...
const actions = [
  {
    ...
    async run(p, ctx) {
      ...
      else value = Number(await ctx.plugin('voicemeeter').call('get', { name })) ? 0 : 1;
      await ctx.plugin('voicemeeter').call('set', { name, value });
    },
  },
  {
    id: 'vm.gain',
    ...
    async run(p, ctx) {
      ...
      if (p.mode === 'reset') await ctx.plugin('voicemeeter').call('set', { name: `${base}.Gain`, value: 0 });
      else if (p.mode === 'set') await ctx.plugin('voicemeeter').call('set', { name: `${base}.Gain`, value: clamp(db, -60, 12) });
      else if (p.mode === 'fade') await ctx.plugin('voicemeeter').call('script', { text: `${base}.FadeTo=(${clamp(db, -60, 12)},${clamp(Math.round(Number(p.ms) || 0), 0, 120000)});` });
      else {
        const step = clamp(Math.abs(db) || 3, 0, 72);
        await ctx.plugin('voicemeeter').call('script', { text: `${base}.Gain ${p.mode === 'down' ? '-=' : '+='}${step};` });
      }
    },
  },
  {
    ...
    async run(p, ctx) {
      ...
      let on = p.mode === 'on';
      if (p.mode === 'toggle') on = !(await ctx.plugin('voicemeeter').call('macro', { button: b }));
      await ctx.plugin('voicemeeter').call('script', { text: `Command.Button[${b}].State=${on ? 1 : 0};` });
    },
  },
  {
    id: 'vm.command',
    ...
    async run(p, ctx) {
      switch (p.command) {
        case 'Restart': case 'Show': case 'Eject': return ctx.plugin('voicemeeter').call('set', { name: `Command.${p.command}`, value: 1 });
        case 'LockOn': return ctx.plugin('voicemeeter').call('set', { name: 'Command.Lock', value: 1 });
        case 'LockOff': return ctx.plugin('voicemeeter').call('set', { name: 'Command.Lock', value: 0 });
        case 'Preset': return ctx.plugin('voicemeeter').call('set', { name: `Command.Preset[${clamp(Math.round(Number(p.preset) || 1), 1, 32) - 1}].Recall`, value: 1 });
        case 'Save': case 'Load': {
          if (!/\.xml$/i.test(p.file || '')) throw new Error('Choose a file that ends in .xml');
          return ctx.plugin('voicemeeter').call('setString', { name: `Command.${p.command}`, value: p.file });
        }
  ...
  },
  {
    ...
    async run(p, ctx) {
      await ctx.plugin('voicemeeter').call('set', { name: 'Command.Reset', value: 1 });
    },
  },
  {
    id: 'vm.start',
    ...
    async run(p, ctx) {
      ...
      await ctx.plugin('voicemeeter').call('run', { type: e + 3 }); // 4-6 are the 64-bit programs
    },
  },
  {
    id: 'vm.device',
    ...
    async run(p, ctx) {
      ...
      await ctx.plugin('voicemeeter').call('setString', { name: `${base}.${p.driver}`, value: p.device || '' });
    },
  },
  {
    id: 'vm.appVolume',
    ...
    async run(p, ctx) {
      ...
      if (p.what === 'gain') await ctx.plugin('voicemeeter').call('script', { text: `${strip}.AppGain=("${app}",${clamp(Number(p.volume), 0, 100) / 100});` });
      else await ctx.plugin('voicemeeter').call('script', { text: `${strip}.AppMute=("${app}",${p.what === 'mute' ? 1 : 0});` });
    },
  },
  {
    id: 'vm.script',
    ...
    async run(p, ctx) {
      ...
      await ctx.plugin('voicemeeter').call('script', { text: String(p.text).slice(0, 4000) });
    },
  },
];
