...
const actions = [
  {
    ...
    async run(p, ctx) {
      ...
      await ctx.plugin('twitch').call((t) => t.sendChat(p.message));
    },
  },
  {
    id: 'twitch.announce',
    ...
    async run(p, ctx) {
      if (!String(p.message || '').trim()) throw new Error('Type a message first');
      await ctx.plugin('twitch').call((t) => t.announce(p.message, p.color));
    },
  },
  {
    id: 'twitch.chatMode',
    ...
    async run(p, ctx) {
      ...
      if (p.action === 'on') on = true;
        ...
        if (known === undefined) {
          const s = await ctx.plugin('twitch').call((t) => t.getChatSettings());
      ...
      await ctx.plugin('twitch').call((t) => t.setChatSettings(patch));
      ctx.plugin('twitch').refresh();
    },
  },
  {
    id: 'twitch.shield',
    ...
    async run(p, ctx) {
      ...
      if (p.action === 'on') on = true;
        ...
        on = known === undefined ? !(await ctx.plugin('twitch').call((t) => t.getShield())) : !known;
      }
      await ctx.plugin('twitch').call((t) => t.setShield(on));
      ctx.plugin('twitch').refresh();
    },
  },
  {
    ...
    needs: 'twitch',
    params: [],
    state: () => 'twitch.adSoon', // lights up when an ad break is about to start, which is when you would press it
    defaults: { label: 'Snooze ad', icon: '⏰', color: '#6b46c1', colorOn: '#d69e2e' },
    async run(p, ctx) {
      const r = await ctx.plugin('twitch').call((t) => t.snoozeAd());
      ctx.plugin('twitch').refresh();
  ...
  },
  {
    ...
    state: () => 'twitch.adRunning', // lit while a commercial break is playing
    defaults: { label: 'Run ad', icon: '📺', color: '#9b2c2c', colorOn: '#d69e2e', confirm: 'hold' },
    async run(p, ctx) {
      const r = await ctx.plugin('twitch').call((t) => t.startCommercial(Number(p.length)));
      ctx.plugin('twitch').refresh();
  ...
  },
  {
    ...
    async run(p, ctx) {
      await ctx.plugin('twitch').call((t) => t.clearChat());
    },
  },
  {
    id: 'twitch.marker',
    ...
    async run(p, ctx) {
      await ctx.plugin('twitch').call((t) => t.createMarker(p.description));
  ...
  },
  {
    ...
    async run(p, ctx) {
      const r = await ctx.plugin('twitch').call((t) => t.createClip());
  ...
  },
  {
    ...
    async run(p, ctx) {
      const u = await ctx.plugin('twitch').call((t) => t.raid(p.channel));
  ...
  },
  {
    ...
    async run(p, ctx) {
      const u = await ctx.plugin('twitch').call((t) => t.shoutout(p.channel));
  ...
  },
  {
    ...
    async run(p, ctx) {
      await ctx.plugin('twitch').call((t) => t.setChannel({ title: p.title, game: p.category }));
