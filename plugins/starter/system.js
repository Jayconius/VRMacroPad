...
const { parseCombo, MEDIA } = require('../../src/core/keys');
...
const actions = [
  ...
  },
  {
    ...
    // Mute follows the real Windows output mute; Play / Pause follows whether something is playing. The rest are one-shot.
    state: (p) => (p.key === 'volumemute' ? 'audio.out.muted' : p.key === 'playpause' ? 'media.playing' : null),
  ...
  },
  {
    ...
    // A program (.exe) lights the button up while it is running. Files, links and shortcuts have no process to watch.
    state: (p) => { const m = /([^\\/]+\.exe)\s*$/i.exec(String(p.path || '').trim().replace(/^"|"$/g, '')); return m ? `proc=${m[1].toLowerCase()}`: null; },
    defaults: { label: 'Launch', icon: '🚀', color: '#b7791f', colorOn: '#2f855a' },
    async run(p) {
